...
def generate_demographic_contact_network(N, demographic_data, layer_generator='FARZ', layer_info=None, distancing_scales=[], isolation_groups=[], verbose=False):
    ...
    if(layer_info is None):
        ...
        # Define the age groups and desired mean degree for each graph layer
        # Data source: Zhang, J., et al, Changes in contact patterns shape the dynamics of the COVID-19 outbreak in China. 
        # Science, 2020. 368(6498): p. 1481-1486.
        # Using the Number of contacts by demographic characteristics for Shanghai
        layer_info  = { '0-9':   {'ageBrackets': ['0-9'],                   'meanDegree': 11.6, 'meanDegree_CI': (9.2, 14.3) },
                        '10-19': {'ageBrackets': ['10-19'],                 'meanDegree': 27.0, 'meanDegree_CI': (23.1, 30.7) },
                        '20-59': {'ageBrackets': ['20-29', '30-39', '40-49', '50-59'], 
                                                    'meanDegree': ((age_distn_given20to60['20-29']+age_distn_given20to60['30-39'])*22.4 + (age_distn_given20to60['40-49']+age_distn_given20to60['50-59'])*19.9), 
                                                    'meanDegree_CI': ( ((age_distn_given20to60['20-29']+age_distn_given20to60['30-39'])*19.8 + (age_distn_given20to60['40-49']+age_distn_given20to60['50-59'])*17.7), ((age_distn_given20to60['20-29']+age_distn_given20to60['30-39'])*25.9 + (age_distn_given20to60['40-49']+age_distn_given20to60['50-59'])*23.3) ) },
                        # '20-39': {'ageBrackets': ['20-29', '30-39'],        'meanDegree': 22.4, 'meanDegree_CI': (19.8, 25.9) },
                        # '40-59': {'ageBrackets': ['40-49', '50-59'],        'meanDegree': 19.9, 'meanDegree_CI': (17.7, 23.3) },
                        '60+':   {'ageBrackets': ['60-69', '70-79', '80+'], 'meanDegree': 12.6, 'meanDegree_CI': (10.8, 14.7) } }
    ...
    if(len(isolation_groups) > 0):
        ...
        for graphName in list(graphs.keys()):
            for graph in list(graphs.values()):
                A_withIsolation = scipy.sparse.csr_matrix.multiply( networkx.adj_matrix(graph), A_isolation_mask )
                graphs[graphName+'_isolation'] = networkx.from_scipy_sparse_matrix(A_withIsolation)
